...
(function () {
  ...
  function calc(v) {
    var kg = v.unit === "metric" ? v.weight : v.weight * 0.45359237;
    var cm = v.unit === "metric" ? v.height : v.height * 2.54;
    if (!(kg > 0 && cm > 0 && v.age > 0)) return null;
    var bmr = 10 * kg + 6.25 * cm - 5 * v.age + (v.sex === "male" ? 5 : -161);
    var tdee = bmr * v.activity;
    var g = GOALS[v.goal];
    var cal = tdee * g.adjust;
    var protein = kg * g.protein;
    var fat = cal * 0.3 / 9;
    var carbs = Math.max(0, (cal - protein * 4 - fat * 9) / 4);
    return {
  ...
  function verdict(f, target, m) {
    ...
    return "Fits your " + target.label + " numbers: " + m.cal.toLocaleString() +
      " of " + target.cal.toLocaleString() + " kcal and " +
      (m.p === null ? "unpublished" : m.p + " g of " + target.protein + " g") + " protein.";
  }
