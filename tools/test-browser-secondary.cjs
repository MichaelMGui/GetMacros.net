...
(async()=>{
 ...
 await p.locator('[data-meal-filter="breakfast"]').click();assert.equal(await p.locator('.meal-idea:visible').count(),2);
 await p.locator('[data-meal-filter="all"]').click();assert.equal(await p.locator('.meal-idea:visible').count(),6);
 console.log('PASS meal examples: visible without a form, breakfast filter, reset');
